...
> **アプリは起動するが読み込み画面のまま進まない場合（サイドカーが起動しない）：** Gatekeeperでアプリ本体を許可しても、内部のPythonサイドカーバイナリにmacOSのquarantine属性が残り、実行がブロックされることがあります。ターミナルで以下のコマンドを実行してquarantine属性を除去してから、アプリを再起動してください：
>
> ```bash
